...
ChatLogic::~ChatLogic()  // required for raw pointers only. Automatically deallocated when smart pointer goes out of scope. 
{
    //// STUDENT CODE
    ////
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));   //transferring ownership of child edge
                        }
                        ////
                        //// EOF STUDENT CODE
                    }
    ...
    ChatBot ChatInPut("../images/chatbot.png");  // on stack : smart ptr/new would create on heap
    ChatInPut.SetRootNode(rootNode);
    ChatInPut.SetChatLogicHandle(this);
    rootNode->MoveChatbotHere(std::move(ChatInPut)); // chatbot instance moved from chatlogic
    ////
    //// EOF STUDENT CODE
}
